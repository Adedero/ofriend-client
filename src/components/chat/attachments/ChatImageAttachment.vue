      ...
      <Carousel v-if="isImageAvailable" :value="images" :numVisible="1" :numScroll="1">
        <template #item="slotProps">
          <div class="grid place-content-center gap-2 max-w-96">
            <img :src="slotProps.data.imageURL" :alt="slotProps.data.name" class="w-full h-80 object-cover">
            <InputText v-model="slotProps.data.caption" placeholder="Caption (optional)" class="w-full" />
          </div>
        </template>
      </Carousel>

      <Button @click="sendFiles" label="Send" severity="secondary" icon="pi pi-send" class="w-full" />
    </div>
  </Dialog>

  <div>
    <label class="cursor-pointer flex flex-col items-center" for="images">
      <span class="p-3 aspect-square rounded-full grid place-content-center bg-sky-400 text-white pi pi-images"></span>
