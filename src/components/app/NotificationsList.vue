...
const loadMoreNotifications = () => useGet(`api/get-notifications?skip=${notificationsLength.value}&limit=${limit}`,
  { router, toast },
  (alerts) => {
...
const clearAllNotifications = () => {
  ...
  loading.value = true;
  usePost('api/clear-all-notifications', { method: 'DELETE', router, toast }, () => {
    notifications.value = [];
    loading.value = false;
  });
}
...
<template>
  <div>
    ...
    <div class="flex items-center justify-between gap-2">
      ...
      <Button v-show="notifications.length > 0" @click="confirmClear()" :loading label="Clear all" icon="pi pi-trash" severity="danger" size="small"
        text />
    </div>
